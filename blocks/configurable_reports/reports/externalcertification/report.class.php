...
class report_externalcertification extends report_base{
	...
	function get_rows($elements, $sqlorder = ''){
		...
		if(!empty($elements)){			
			...
			$records = [1,2,3,4,5,6,11,11,22,33,44,4,5,5,6,6,7,7,78,8];
			...
			foreach($records as $record){
				...
				$columns->approveddate = "Fiscal Year- Apr-Mar";
				$columns->employeeemail  = "NA";
				$columns->employeegrade  = "Master Sheet will be given (Apex+ & Apex)";
				$columns->employeedepartment  = "Master Sheet will give (Delivery & GCF";
				$columns->coursecredits  = "5";
				$columns->certificationname  = "Certification Name";
